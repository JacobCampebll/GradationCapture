...
async function loadClientLogic() {
  ...
  const names = ["num", "round1", "calcSample", "sampleChecks", "sampleHasData",
    ...
    "gramsColumnCells", "openRuns", "profilePeaks", "longestUniformRun", "findPageBox"];
...
/* ================= segmentation: refuse rather than guess ================= */
// A wrong cell rectangle yields confident nonsense that the arithmetic checks
// cannot distinguish from an odd sheet. Every one of these must come back ok:false.
describe("gramsColumnCells — geometry it should reject outright", () => {
  const PAGE = { x0: 0, y0: 0, x1: 1000, y1: 800, w: 1000, h: 800 };
  const evenly = (from, to, n) =>
    Array.from({ length: n }, (_, i) => from + (to - from) * i / (n - 1));

  // 16 rules bounding the 15 data rows, and 10 bounding the 9 gradation columns.
  const goodGrid = () => ({ page: PAGE, rows: evenly(150, 620, 16), cols: evenly(180, 640, 10) });

  test("a well-formed grid produces one cell per row, in order", () => {
    const r = app.gramsColumnCells(goodGrid(), 0, CONFIG.sieves);
    assert.equal(r.ok, true, r.reason);
    assert.equal(r.cells.length, CONFIG.sieves.length + 2);
    assert.equal(r.cells[0].key, CONFIG.sieves[0].key);
    assert.equal(r.cells[r.cells.length - 2].key, "pan");
    assert.equal(r.cells[r.cells.length - 1].key, "totalWt");
    for (let i = 1; i < r.cells.length; i++) {
      assert.ok(r.cells[i].y > r.cells[i - 1].y, "cells must run down the page in order");
    }
  });

  test("the struck 6mm row is carried but marked skip", () => {
    const r = app.gramsColumnCells(goodGrid(), 0, CONFIG.sieves);
    const struck = CONFIG.sieves.findIndex((s) => s.skip);
    assert.equal(r.cells[struck].skip, true);
    assert.equal(r.cells.filter((c) => c.skip).length, 1);
  });

  test("samples 2 and 3 read from their own columns, further right", () => {
    const a = app.gramsColumnCells(goodGrid(), 0, CONFIG.sieves);
    const b = app.gramsColumnCells(goodGrid(), 1, CONFIG.sieves);
    const c = app.gramsColumnCells(goodGrid(), 2, CONFIG.sieves);
    assert.ok(b.ok && c.ok);
    assert.ok(b.cells[0].x > a.cells[0].x);
    assert.ok(c.cells[0].x > b.cells[0].x);
  });

  test("too few row rules is a refusal, not a stretch to fit", () => {
    const g = goodGrid(); g.rows = evenly(150, 620, 9);
    const r = app.gramsColumnCells(g, 0, CONFIG.sieves);
    assert.equal(r.ok, false);
    assert.equal(r.cells.length, 0);
    assert.match(r.reason, /row rules/);
  });

  test("too few column rules is a refusal — we cannot know which column is Grams", () => {
    const g = goodGrid(); g.cols = evenly(180, 640, 3);
    const r = app.gramsColumnCells(g, 0, CONFIG.sieves);
    assert.equal(r.ok, false);
    assert.match(r.reason, /column rules/);
  });

  test("a sliver of a column is a refusal (a doubled page border, not a cell)", () => {
    const g = goodGrid(); g.cols = evenly(180, 250, 10); // 7px columns
    const r = app.gramsColumnCells(g, 0, CONFIG.sieves);
    assert.equal(r.ok, false);
    assert.match(r.reason, /not plausible/);
  });

  test("rules spanning almost the whole page are not the gradations table", () => {
    const g = goodGrid(); g.rows = evenly(10, 795, 16);
    const r = app.gramsColumnCells(g, 0, CONFIG.sieves);
    assert.equal(r.ok, false);
    assert.match(r.reason, /gradations table/);
  });

  test("cells taller than they are wide are the wrong shape for this form", () => {
    // Sized so the column-width and table-span guards both pass and only the
    // aspect ratio is wrong: 60px wide cells against 60px tall ones.
    const tall = { x0: 0, y0: 0, x1: 1000, y1: 1200, w: 1000, h: 1200 };
    const g = { page: tall, rows: evenly(100, 1000, 16), cols: evenly(180, 720, 10) };
    const r = app.gramsColumnCells(g, 0, CONFIG.sieves);
    assert.equal(r.ok, false);
    assert.match(r.reason, /wrong shape/);
  });
});
